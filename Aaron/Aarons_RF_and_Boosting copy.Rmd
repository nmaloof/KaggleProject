...
train_indices = sample(1:nrow(train4), 8*nrow(train4)/10)
...
sqrt(mean((yhat4 - train4_test$SalePrice)^2)) # --> 0.141765; kaggle sub2 = 0.14614
...
write.csv(FINAL, "sub2.csv", row.names = F)
...
abc = predict(gbmFit, newdata = train4_test)
sqrt(mean((abc - train4_test$SalePrice)^2)) # 0.1418164
pred = exp(abc) + 1


# ----- using gbm with the params from gbmFit
...
gbm_best = predict(boost.train.best, newdata = train4_test, n.trees = 500)

sqrt(mean((gbm_best - train4_test$SalePrice)^2)) # 0.132535; kaggle: 0.12581
...
write.csv(FINAL.boost, "sub15.csv")
